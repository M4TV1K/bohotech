...
window.onload = () => {
    ...
    $('#extendButton').click(() => {
        if (navBar.style.display === 'none') {
            ...
            $('#headerLogo').attr('src', 'Images/Methodpage/HeaderLogo.png');
            $('#extendButton').css('background-image', 'url("Images/Methodpage/ExtendButton.png")');
            $('main').css({
                'filter': 'blur(30px)',
                'margin-top': '12vw'
            });
...
function resizeWindow() {
    if (window.innerWidth > 890) {
        ...
        $('#headerLogo').attr('src', 'Images/Homepage/HeaderLogo.png');
        $('#extendButton').css('background-image', 'url("Images/Homepage/ExtendButton.png")');
        $('main').css({
            'filter': 'none',
            'margin-top': '0'
        });
